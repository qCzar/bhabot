...
function Announcement(meetup: db.Meetup, rsvps?: string[], maybes?: string[]): EmbedBuilder {
   ...
   if (meetup.duration) {
      embed.addFields ({ name: "Duration", value: `${meetup.duration} hour(s)` });
   }

   if (meetup.rsvpDeadline) {
      embed.addFields ({ name: "⏰ RSVP Deadline", value: Format.time (DateTime.fromISO (meetup.rsvpDeadline), Format.TimeFormat.Full) });
   }
   ...
   rsvps && embed.addFields ({
      name:  meetup.maxRsvp
         ? `✅ Attending ${withCount (rsvps.length)} / ${meetup.maxRsvp}`
