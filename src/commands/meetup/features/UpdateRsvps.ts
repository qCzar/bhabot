...
const createRsvpListener = async (client: Discord.Client, meetup: db.Meetup): Promise<Unbind> => {
   ...
   collector.on ("collect", async i => {
      ...
      runWithLoggingContext (async () => {
         // Enforce RSVP restrictions
         if (meetupState.rsvpDeadline && DateTime.local () > DateTime.fromISO (meetupState.rsvpDeadline)) {
            i.reply ({ content: "⏰ The RSVP deadline has passed.", ephemeral: true });
            return;
         }

         if (i.customId === "rsvp" && !meetupState.rsvps.includes (i.user.id) && meetupState.maxRsvp && meetupState.rsvps.length >= meetupState.maxRsvp) {
            i.reply ({ content: "😔 This meetup is full!", ephemeral: true });
            return;
         }
